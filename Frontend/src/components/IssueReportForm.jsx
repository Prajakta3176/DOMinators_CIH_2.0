// ✅ Updated IssueForm.jsx (frontend)
...
export default function IssueForm() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, lat, lng, city }));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:8080/api/user/citizen/addComplaint",
        {
    ...
    <section className="flex justify-center items-center flex-col h-full w-full bg-[linear-gradient(to_bottom,#06ACF180,#FFFFFF82)]">
      <div className="space-y-6 p-6 max-w-xl mx-auto bg-[#fefeff] rounded-2xl w-[80vw] shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <Label htmlFor="image" className="text-blue-900 font-medium">Upload Image (not sent)</Label>
